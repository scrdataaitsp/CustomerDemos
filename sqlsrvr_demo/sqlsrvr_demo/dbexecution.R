...
sqlConnString <- "Driver=SQL Server;Server=srramsqlml.southcentralus.cloudapp.azure.com; Database=demo;Uid=srramadmin;Pwd=Lz8oq1dn$Lz1"
sqlWait <- TRUE
sqlConsoleOutput <- FALSE
sqlShareDir <- paste("C:\\Users\\srram\\Documents\\AllShare\\", Sys.getenv("USERNAME"), sep = "")
...
rxSummary( ~ ., data = sqlDSTransformed,computeContext = sqlComputeTrace)
...
DforestModelObj <- rxDForest(value ~ RDPI + year + month + weekofmonth +
    ...
    lag21 + lag22 + lag23 + lag24 + lag25 + lag26, data = sqlDSTransformed ,computeContext = sqlComputeTrace)
...
sqlServerOutDS <- RxSqlServerData(table = "Predictions",
    connectionString = sqlConnString,
    rowsPerRead = 5000)
...
rxPredict(modelObject = DforestModelObj,
            data = sqlDSTest,
